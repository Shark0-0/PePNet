...
class PSTConv(nn.Module):
    def __init__(self,
        ...
        self.spatial_neighbours = self.k
        ...
        temporal_channels = [out_planes]

        self.spatial_aggregation = spatial_aggregation
        self.spatial_pooling = spatial_pooling

        if in_planes != 0:
            self.spatial_conv_f = nn.Conv2d(in_channels=in_planes * 2 + 3, out_channels=mid_planes, kernel_size=1, stride=1, padding=0, bias=bias)
            ...
            self.spatial_conv_f = nn.Conv2d(in_channels=3, out_channels=mid_planes, kernel_size=1, stride=1, padding=0, bias=bias)
            kaiming_uniform(self.spatial_conv_f.weight, size=[mid_planes, in_planes+3, 1, 1])
            if bias:
                bound = 1 / math.sqrt(in_planes+3)
                uniform(self.spatial_conv_f.bias, -bound, bound)

        for t in range(2*self.temporal_radius + 1):
            temporal_mlp = []
            for i in range(len(temporal_channels)):

                temporal_mlp.append(nn.Conv1d(in_channels=mid_planes, out_channels=out_planes, kernel_size=1, stride=1, padding=0, bias=bias))

                temporal_mlp.append(nn.BatchNorm1d(num_features=out_planes))
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        if self.padding_mode == "zeros":
            xyz_padding = torch.zeros(xyzs[0].size(), dtype=torch.float32, device='cuda')
        ...
        for t in range(self.temporal_radius, len(xyzs)-self.temporal_radius, self.temporal_stride):                                 # temporal anchor frames
            ...
            for j, i in enumerate(range(t-self.temporal_radius, t+self.temporal_radius+1)):
                ...
                displacement = (neighbor_xyz_grouped - anchor_xyz_expanded)/self.r                                                         # (B, 3, N//spatial_stride, k)
                # displacement = self.spatial_conv_d(displacement)                                                                    # (B, mid_planes, N//spatial_stride, k)

                if self.in_planes != 0:

                    neighbor_feature_grouped = pointnet2_utils.grouping_operation(features[i],
                                                                                  idx)  # (B, in_channels, N//spatial_sampling, spatial_neighbours)
                    center_idx = anchor_idx.unsqueeze(-1)
                    center_idx = center_idx.repeat(1, 1, self.spatial_neighbours)
                    center_feature = pointnet2_utils.grouping_operation(features[t], center_idx)
                    spatial_feature = torch.cat((displacement, neighbor_feature_grouped, center_feature),
                                                dim=1)  # (B, 3+in_channels, N//spatial_sampling, spatial_neighbours)

                else:
                    spatial_feature = displacement

                spatial_feature = self.spatial_conv_f(spatial_feature)                                                                 # (B, spatial_channels[-1], N//spatial_sampling, spatial_neighbours)
